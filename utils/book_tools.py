...
def parse_tool_output(output: str, expected_type: str = "unknown") -> dict:
    try:
        import json

        parsed = json.loads(output)
        logger.info(f"Successfully parsed {expected_type} output")
        return parsed
    except json.JSONDecodeError as e:
        logger.error(f"Failed to parse {expected_type} JSON: {str(e)}")
        return {
...
def format_structured_output_for_llm(parsed_output: dict) -> str:
    try:
        if "error_type" in parsed_output:
            return f"Tool Error ({parsed_output['error_type']}): {parsed_output.get('error_message', 'Unknown error')}"

        if "books" in parsed_output:
            result = f"Found {parsed_output.get('total_found', 0)} books for '{parsed_output.get('query', 'unknown query')}':\n\n"
            for i, book in enumerate(parsed_output.get("books", []), 1):
                result += f"{i}. **{book.get('title', 'Unknown Title')}**\n"
                if book.get("authors"):
                    result += f"   Authors: {', '.join(book['authors'])}\n"
                if book.get("publish_year"):
                    result += f"   Published: {book['publish_year']}\n"
                if book.get("subjects"):
                    result += f"   Subjects: {', '.join(book['subjects'][:3])}\n"
                result += "\n"
            return result

        if "documents" in parsed_output:
            result = f"Retrieved {parsed_output.get('total_documents', 0)} knowledge sections for '{parsed_output.get('query', 'unknown query')}':\n\n"
            if parsed_output.get("summary"):
                result += f"**Summary:** {parsed_output['summary']}\n\n"

            for i, doc in enumerate(parsed_output.get("documents", []), 1):
                result += (
                    f"**Section {i}:**\n{doc.get('content', 'No content')[:300]}...\n\n"
                )
            return result

        if "recommendations" in parsed_output:
            result = f"Generated {parsed_output.get('total_recommendations', 0)} recommendations for '{parsed_output.get('query', 'unknown preferences')}':\n\n"

            if parsed_output.get("context"):
                result += f"**Context:** {parsed_output['context']}\n\n"

            result += "**Recommendations:**\n"
            for i, rec in enumerate(parsed_output.get("recommendations", []), 1):
                result += f"{i}. **{rec.get('title', 'Unknown Title')}**\n"
                if rec.get("authors"):
                    result += f"   Authors: {', '.join(rec['authors'])}\n"
                if rec.get("publish_year"):
                    result += f"   Published: {rec['publish_year']}\n"
                if rec.get("reason"):
                    result += f"   Why recommended: {rec['reason']}\n"
                result += "\n"
            return result

        return f"Structured output received: {str(parsed_output)}"

    except Exception as e:
        logger.error(f"Error formatting structured output: {str(e)}")
        return f"Error formatting tool output: {str(e)}"
